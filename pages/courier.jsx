...
export default function CourierPage() {
  ...
  <>
    ...
    <table className={styles.table}>
      <thead>
        <tr>
          <th>Клуб</th><th>Описание</th><th>ПК</th><th>Тип</th><th>Модель</th><th>Статус</th><th>Дата</th><th>Действие</th>
        </tr>
      </thead>
      <tbody>
        {filteredRepairs.filter((r) => !r.approved).map((r) => (
          <tr key={r.id}>
            <td>{r.club_address}</td><td>{r.description}</td><td>{r.pc_number}</td>
            <td>{r.equipment_type}</td><td>{r.model}</td><td>{r.status}</td><td>{formatDateTime(r.created_at)}</td>
            <td>
              <button className={styles.confirmButton} onClick={() => handleConfirm(r.id)}>Подтвердить</button>
              <button className={styles.returnButton} onClick={() => handleCancel(r.id)}>Отменить</button>
            </td>
          </tr>
        ))}
      </tbody>
      ...
      <tbody>
        {filteredRepairs.filter((r) => r.approved).map((r) => (
          <tr key={r.id}>
            ...
            <td>{r.equipment_type}</td><td>{r.model}</td><td>{r.status}</td><td>{formatDateTime(r.created_at)}</td>
            <td>
              <button className={styles.confirmButton} onClick={() => handleMarkInRepair(r.id)}>Доставил</button>
            </td>
          </tr>
        ))}
      </tbody>
...
{tab === 'delivery' && (
  <>
    ...
    <table className={styles.table}>
      <thead>
        <tr><th>Клуб</th><th>Описание</th><th>ПК</th><th>Тип</th><th>Модель</th><th>Статус</th><th>Дата</th></tr>
      </thead>
      <tbody>
    ...
    <table className={styles.table}>
      <thead>
        <tr><th>Клуб</th><th>Описание</th><th>ПК</th><th>Тип</th><th>Модель</th><th>Статус</th><th>Дата</th><th>Действие</th></tr>
      </thead>
      <tbody>
        {filteredAccepted.map((r) => (
          <tr key={r.id}>
            ...
            <td>{r.equipment_type}</td><td>{r.model}</td><td>{r.status}</td><td>{formatDateTime(r.created_at)}</td>
            <td>
              <button className={styles.confirmButton} onClick={() => handleClose(r.id)}>Отдал</button>
            </td>
          </tr>
        ))}
      </tbody>
